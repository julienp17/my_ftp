...
reply_code cmd_user(server_t *server, client_t *client, char *arg)
{
    ...
    if (client->auth == LOGGED_IN) {
        ...
        send_reply(client->sock, code, "Can't change from guest user.");
    } else {
        if (arg == NULL) {
            code = RPL_NOT_LOGGED_IN;
            send_reply(client->sock, code, "Permission denied.");
        } else {
            code = RPL_USERNAME_OK;
            client->auth = USERNAME_ENTERED;
            client->username = strdup(arg);
            send_reply(client->sock, code, "Please specify the password.");
        }
    }
    return code;
}